...
int AX12A::moveSpeedRW(unsigned char ID, int Position, int Speed)
{
 ...
 void AX12A::bulkReadPosition(ServoPosition* positions, unsigned char* IDs, int numServos) {
        if(numServos <= 0 || numServos > MAX_SERVOS) return;
        
        // Calculate packet length: 
        // 4 (header + length) + numServos * 3 (ID + start addr + length) + 1 (checksum)
        const unsigned int length = 4 + (numServos * 3) + 1;
        unsigned char packet[length];
        ...
        packet[0] = AX_START;
        packet[1] = AX_START;
        packet[2] = BROADCAST_ID;  // Bulk read uses broadcast ID
        packet[3] = length - 4;    // Length field excludes first 4 bytes
        
        // Parameter data for each servo
        int idx = 4;
        for(int i = 0; i < numServos; i++) {
            packet[idx++] = IDs[i];                    // Servo ID
            packet[idx++] = AX_PRESENT_POSITION_L;     // Start address
            packet[idx++] = AX_BYTE_READ_POS;         // Length of data to read
            
            // Add to checksum
            checksum += IDs[i] + AX_PRESENT_POSITION_L + AX_BYTE_READ_POS;
        }
        
        // Calculate final checksum
        checksum = (~(BROADCAST_ID + (length - 4) + AX_BULK_READ + checksum)) & 0xFF;
        packet[idx] = checksum;
        
        // Send bulk read command
        ...
        // Wait for responses
        Time_Counter = 0;
        while((availableData() < (numServos * 6)) && (Time_Counter < TIME_OUT)) {
            Time_Counter++;
            delayus(1000);
        }
        
        // Read responses for all servos
        for(int i = 0; i < numServos; i++) {
            // Wait for and verify header
            while(availableData() > 0) {
                Incoming_Byte = readData();
                if((Incoming_Byte == 255) & (peekData() == 255)) {
                    readData();                            // Second start byte
                    unsigned char receivedID = readData(); // Servo ID
                    readData();                           // Length
                    
                    // Find matching servo in our array
                    for(int j = 0; j < numServos; j++) {
                        if(positions[j].ID == receivedID) {
                            if((Error_Byte = readData()) != 0) {  // Error
                                positions[j].error = Error_Byte;
                            } else {
                                // Read position
                                Position_Low_Byte = readData();
                                Position_High_Byte = readData();
                                positions[j].position = (Position_High_Byte << 8) + Position_Low_Byte;
                            }
                            break;
                        }
                    }
...
int AX12A::syncWritePosition(unsigned char IDs[], int positions[], int speeds[], int numServos) {
    // Use a static buffer instead of malloc to avoid memory allocation issues
    static unsigned char parameter[128]; // Should be enough for most cases
    
    // Calculate packet length - each servo needs 5 bytes
    uint8_t lengthValue = 4 + (numServos * 5);
    
    // Check if we need a buffer larger than what we allocated
    if (lengthValue + 8 > sizeof(parameter)) { // +8 for header, etc.
        Serial.println(F("Error: Too many servos for buffer"));
        return -1;
    }
    
    uint8_t temp = 0;
    int checksum = 254 + lengthValue + AX_SYNC_WRITE + 4 + AX_GOAL_POSITION_L;
    
    // Build the packet header
    parameter[temp++] = AX_START;
    parameter[temp++] = AX_START;
    parameter[temp++] = BROADCAST_ID;
    parameter[temp++] = lengthValue;
    parameter[temp++] = AX_SYNC_WRITE;
    parameter[temp++] = AX_GOAL_POSITION_L;  // Starting address
    parameter[temp++] = 4;  // Length of data per servo (pos[2] + speed[2])
    
    // Add servo data
    for (int i = 0; i < numServos; i++) {
        parameter[temp++] = IDs[i];
        parameter[temp++] = positions[i] & 0xFF;         // Low byte
        parameter[temp++] = (positions[i] >> 8) & 0xFF;  // High byte
        parameter[temp++] = speeds[i] & 0xFF;            // Low byte
        parameter[temp++] = (speeds[i] >> 8) & 0xFF;     // High byte
        
        checksum += IDs[i] + (positions[i] & 0xFF) + ((positions[i] >> 8) & 0xFF) + 
                   (speeds[i] & 0xFF) + ((speeds[i] >> 8) & 0xFF);
    }
    
    // Add checksum
    parameter[temp++] = ~checksum & 0xFF;
    
    // Send the packet
    sendAXPacketNoError(parameter, temp);
    return 0;
}
...
{
  if (packet == NULL) {
    Serial.println("Error: Null packet in sendAXPacketNoError");
    return;
}
